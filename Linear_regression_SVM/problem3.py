...
def gaussian_kernel(X1, X2,gamma=1.):
    ...
    n1 = Prod.shape[0]
    n2 = Prod.shape[1]
    X1sq = np.multiply(X1,X1).sum(axis=1)
    X1sq = np.tile(X1sq,(1,n2))
    X2sq = np.multiply(X2,X2).sum(axis=1)
    X2sq = np.tile(X2sq.T,(n1,1))
    K = X1sq - 2 * Prod + X2sq
    K = np.exp(-K/(2*gamma**2))
