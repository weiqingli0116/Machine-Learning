...
def compute_distance(Xtrain, Xtest):
    ...
    prod = Xtest.dot(Xtrain.T)
    n_train = Xtrain.shape[0]
    n_test = Xtest.shape[0]
    trsq = (Xtrain*Xtrain).sum(axis=1)
    trsq = np.tile(trsq,(n_test,1))
    tesq = (Xtest*Xtest).sum(axis = 1)
    tesq = np.tile(tesq,(n_train,1)).T
    D = trsq - 2*prod +tesq
    D = np.sqrt(D)
...
def k_nearest_neighbor(Xtrain, Ytrain, Xtest, K = 3):
    ...
    Y = np.argsort(D,axis = 1)[:,0:K]
    Ytest = []
    for row in Y:
        Yrow = []
        Yrow = [Ytrain[element] for element in row]
        Ytest.append(Counter(Yrow).most_common(1)[0][0])
