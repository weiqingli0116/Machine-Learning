...
def compute_a(z):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    func = lambda x: np.exp(x) if x > -500 and x < 500 else 0 if x <= -500 else float('Inf')
    c = z.shape[0]
    a = np.matrix(np.ones((c,1)))
    expz = np.matrix(np.zeros((c,1)))
    for i in range(c):
        expz[i,0] = func(z[i,0])
    s = expz.sum()
    if s == float('inf'):
        if expz.min() == float('inf'):
            a = (1. /c) * a
        else:
            for i in range(c):
                a[i,0] = 0 if expz[i,0] < float('inf') else 1
    elif s == 0:
        a = (1. /c) * a
    else:
        a = expz / s
...
def compute_L(a,y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    L = float(-np.log(a[y,0])) if a[y,0] != 0 else 1e7
...
def compute_dL_da(a, y):
    ...
    dL_da[y,0] = -1/a[y,0] if a[y,0] != 0 else -1e5-1
...
def compute_da_dz(a):
    ...
    ## INSERT YOUR CODE HERE
    prod = np.dot(a, a.T)
    da_dz = np.matrix(np.diag(np.matrix.tolist(a.T)[0])) - prod
...
def compute_dL_dW(dL_dz,dz_dW):
    ...
    ## INSERT YOUR CODE HERE
    p = dz_dW.shape[1]
    dL_dW = np.multiply(np.tile(dL_dz,(1,p)), dz_dW)
